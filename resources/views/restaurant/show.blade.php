...
<div class="min-h-screen bg-white text-ink">
    ...
    <div class="relative h-[350px] md:h-[450px] overflow-hidden bg-brand-black">
        {{-- Image de couverture du restaurant (ou fond noir sobre par défaut) --}}
        @if($restaurant->cover_image)
            <img src="{{ asset('storage/' . $restaurant->cover_image) }}" alt="{{ $restaurant->name }}" class="w-full h-full object-cover opacity-80">
        @else
        ...
        <div class="absolute bottom-0 inset-x-0 pb-10">
            <div class="container mx-auto px-4 md:px-8">
                <a href="{{ route('home') }}" class="inline-flex items-center gap-2 text-xs uppercase tracking-widest font-semibold text-white hover:text-brand-red transition-colors mb-4 group">
                    ...
                    @forelse($restaurant->menus as $menu)
                        <div class="plat-card bg-white border border-border hover:border-brand-black rounded-md p-4 flex flex-col justify-between transition-all duration-300 group">
                            <div class="flex gap-4">
                                {{-- Image du plat ou icône sobre par défaut --}}
                                <div class="w-20 h-20 bg-[#EFEFEC] rounded-sm overflow-hidden flex-shrink-0 border border-border flex items-center justify-center">
                                    @if($menu->image)
                                        <img src="{{ asset('storage/' . $menu->image) }}" alt="{{ $menu->name }}" class="w-full h-full object-cover grayscale-[0.15] group-hover:scale-105 transition-transform duration-500">
                                    @else
